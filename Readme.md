...
It just means that items of that combination will route by the higher rule order. There may be other FLICKTUNE types
that may need to match and other CDs that need to match on other locations.
TODO: Fix scraping and parsing rules correctly.
TODO: Complete XML config file handling - maybe even importing as well.
